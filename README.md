...
n-gram melody baseline trained on synthetic bootstrap data (see
[Stage 2](#stage-2-n-gram-melody-baseline) below) — not a neural model,
and not trained on real music.
...
src/creative_audio_lab/
    ...
    models/                         GenerationBackend interface, DeterministicBackend (default),
                                    ...
                                    + n-gram model/training utilities,
...
scripts/
    ...
    train_ngram_melody.py           Train + save the Stage 2 n-gram melody model
...
- **It replaces only the melody.** The deterministic pipeline still parses
  ...
  trained n-gram model (configurable order, default 3, with backoff,
  temperature, and top-k sampling) over symbolic melody tokens, decoded
  back into notes through the existing REMI-style tokenizer.
- **It trains on synthetic bootstrap data by default.** The demo model is
  ...
  statistics pool across keys). That makes it a real fit/sample/serialize
  pipeline — and *not* evidence of learning from real music. To train on an
  actual corpus, tokenize your own licensed MIDI locally and pass the
  sequences to `train_ngram_model`, or point the backend at a saved model:
  `NgramMelodyBackend(model_path="my_model.json")`. Nothing is downloaded.
...
Train and save a model, or compare the backends side by side:

```bash
python scripts/train_ngram_melody.py --output ngram_melody.json
python scripts/compare_backends.py            # Markdown metric table
```

The comparison reports note density, harmonic fit, novelty, repetition,
scale adherence, motif retention (versus the deterministic melody for the
same prompt), note counts, and MIDI-export validity per backend. Typical
results: the n-gram melody is *more novel and less repetitive* but has
*lower harmonic fit and scale adherence* than the rule-based melody —
real trade-offs, reported as such.
...
- The Stage 2 n-gram melody baseline samples from statistics of a
  synthetic corpus generated by the deterministic backend, so it inherits
  that generator's stylistic range; its melodies wander harmonically more
  than the rule-based ones (see `scripts/compare_backends.py`).
